...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        self.hybrid = None
        self.config = config
        # img_size = _triple(img_size)       #   _pair
        # print(img_size)       # (img_size, img_size, img_size)

        if config.patches.get("grid") is not None:   # ResNet
            grid_size = config.patches["grid"]
            # print('grid_size:', grid_size)   # (img_size/patch_size, img_size/patch_size, img_size/patch_size)
        ...
        self.patch_embeddings = Conv3d(in_channels=in_channels,        #  + config.skip_channels[0] + config.skip_channels[1] + config.skip_channels[2]
                                       ...
                                       stride=patch_size)
        self.position_embeddings = nn.Parameter(torch.randn(1, n_patches, config.hidden_size))
...
class Block(nn.Module):
    ...
    def forward(self, x):
        h = x                                              #
        x = self.attention_norm(x)                         # these two code may be exchanged
...
class DecoderResBlock(nn.Module):
    def __init__(
        ...
        self.se_block = SELayer(channel=out_channels)
...
class DecoderCup(nn.Module):
    ...
    def forward(self, hidden_states, features=None, input=None):
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.decoder(x, features, input)
    ...
    def load_from(self, weights):
        ...
        self.transformer.encoder.encoder_norm.weight = nn.Parameter(np2th(weights["Transformer/encoder_norm/scale"]).cuda())
        self.transformer.encoder.encoder_norm.bias = nn.Parameter(np2th(weights["Transformer/encoder_norm/bias"]).cuda())

        '''
        posemb = np2th(weights["Transformer/posembed_input/pos_embedding"])

        posemb_new = self.transformer.embeddings.position_embeddings
        # print('origin and new positional embedding:', posemb.shape, posemb_new.shape)     # torch.Size([1, 197, 768]) torch.Size([1, 64, 768])
        if posemb.size() == posemb_new.size():
            ...
            # print('new positional embedding:', ntok_new)      # 64 = 4 * 4 * 4
            if self.classifier == "seg":
                _, posemb_grid = posemb[:, :1], posemb[0, 1:]
            gs_old = int(np.sqrt(len(posemb_grid)))                ## len(posemb_grid): the row of matrix posemb_grid
            gs_new = int(np.cbrt(ntok_new))
            print('load_pretrained: grid-size from %s to %s' % (gs_old, gs_new))
            posemb_grid = posemb_grid.reshape(gs_old, gs_old, -1)
            zoom = (gs_new / gs_old, gs_new / gs_old, 1)
            posemb_grid = ndimage.zoom(posemb_grid, zoom, order=1)  # th2np
            posemb_grid = posemb_grid.reshape(1, gs_new * gs_new * gs_new, -1)
            posemb = posemb_grid
...
def main():
    ...
    config_vit.n_patches = int(args.img_size[0] / args.vit_patches_size) * int(
        args.img_size[1] / args.vit_patches_size) * int(args.img_size[2] / args.vit_patches_size)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_root_path', type=str,
                        default='/home/youxin/PycharmProjects/project_TransUNet/data/Synapse/train_npz',
                        help='root dir for train data')
    parser.add_argument('--val_root_path', type=str,
                        default='/home/youxin/PycharmProjects/project_TransUNet/data/Synapse/val_npz',
                        help='root dir for val data')
    parser.add_argument('--dataset', type=str,
                        default='Synapse', help='experiment_name')
    parser.add_argument('--list_dir', type=str,
                        default='/home/youxin/PycharmProjects/project_TransUNet/TransUNet/lists/lists_Synapse',
                        help='list dir')
    ...
    parser.add_argument('--img_size', type=int,
                        default=[80, 64, 64], help='input patch size of network input')  # network input image size
