...
class TransformerBTS(nn.Module):
    def __init__(
        ...
        self.num_patches = int(img_dim[0] // patch_dim) * int(img_dim[1] // patch_dim) * int(img_dim[2] // patch_dim)
...
def TransBTS(in_channels=4, out_channels=4, img_dim=None, _conv_repr=True, _pe_type="learned"):
    # img_dim = 128
...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        _, model = TransBTS(in_channels=1, out_channels=out_channel, img_dim=[128, 160, 96], _conv_repr=True, _pe_type="learned")
        model.cuda()

        y = model(x)
        print(y)
        parameter_num = print_network(model)
        print("Total number of network parameters: {}".format(parameter_num))  # 32.99M